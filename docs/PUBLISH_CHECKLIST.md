...
- [x] 릴리스 서명: **전용 신규 키** (SHA1 `9A:04:B6:C2:...`, 2026-07-09 재생성)
  - 백업: 데스크탑 `연금나침반_서명키_백업/` (keystore + 비밀번호 — **절대 분실 금지**)
  - 서명 키 사고 이력(재발 방지용): ①공시한줄 앱 항목에 잘못 업로드해 "키 불일치·패키지명" 반려 2건
    ②어제(7/8) 만든 원래 전용 키(`AC:A0:...`)는 비밀번호 유실로 폐기(`upload-keystore-UNUSED-ACA0.jks.bak`)
    ③Play 연금나침반 항목도 그 키로 초기화돼 있어 **항목 삭제·재생성** + 새 전용 키로 재출발.
    교훈: keystore 생성 즉시 데스크탑 백업 폴더에 비밀번호까지 기록할 것.
