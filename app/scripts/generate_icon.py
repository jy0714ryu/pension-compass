...
if __name__ == "__main__":
    ...
    print("다음 단계: flutter pub get && dart run flutter_launcher_icons")
    arrow_rad2 = rad - math.radians(150)
    arrow_points = [
        (tip_x, tip_y),
        (tip_x + int(arrow_size * math.cos(arrow_rad1)),
         tip_y + int(arrow_size * math.sin(arrow_rad1))),
        (tip_x + int(arrow_size * math.cos(arrow_rad2)),
         tip_y + int(arrow_size * math.sin(arrow_rad2)))
    ]
    draw.polygon(arrow_points, fill=GREEN)
    
    # 중심점 원
    center_dot_radius = int(size * 0.04)
    draw.ellipse(
        [center - center_dot_radius, center - center_dot_radius,
         center + center_dot_radius, center + center_dot_radius],
        fill=WHITE
...
def create_foreground_icon(size=1024, output_path="app_icon_foreground.png"):
    ...
    center = size // 2
    radius = int(size * 0.28)  # Adaptive icon은 더 작게
    
    # 외곽 원 (흰색)
    outer_radius = radius + int(size * 0.015)
    draw.ellipse(
        [center - outer_radius, center - outer_radius,
         center + outer_radius, center + outer_radius],
        outline=WHITE, width=int(size * 0.012)
    )
    ...
    tick_width = int(size * 0.01)
    for angle in [0, 90, 180, 270]:
        rad = math.radians(angle - 90)
        x1 = center + int((radius - tick_length) * math.cos(rad))
        y1 = center + int((radius - tick_length) * math.sin(rad))
        x2 = center + int(radius * math.cos(rad))
        y2 = center + int(radius * math.sin(rad))
        draw.line([(x1, y1), (x2, y2)], fill=WHITE, width=tick_width)
    
    # 나침반 바늘 - 오른쪽 위
    needle_angle = -45
    needle_length = int(radius * 0.7)
    rad = math.radians(needle_angle)
    
    tip_x = center + int(needle_length * math.cos(rad))
    tip_y = center + int(needle_length * math.sin(rad))
    tail_x = center - int(needle_length * 0.3 * math.cos(rad))
    tail_y = center - int(needle_length * 0.3 * math.sin(rad))
    ...
    arrow_rad1 = rad + math.radians(150)
    arrow_rad2 = rad - math.radians(150)
    arrow_points = [
        (tip_x, tip_y),
        (tip_x + int(arrow_size * math.cos(arrow_rad1)),
    ...
    # 중심점
    center_dot_radius = int(size * 0.03)
    draw.ellipse(
        [center - center_dot_radius, center - center_dot_radius,
         center + center_dot_radius, center + center_dot_radius],
        fill=WHITE
